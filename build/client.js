...
class MultiplexSocket extends net_1.Socket {
    constructor(...args) {
        super(...args);
        this.sending = false;
    ...
    emitMessagesFromBuffer(buffer, maxBufferSizeDigits) {
        if (buffer.length === 0) {
            return buffer;
        }
    ...
    multiplexWrite(channelId, buffer) {
...
const sockets = {};
const forwardPortThroughSocket = (socketToPeer, portToForward) => {
    const messagesFor = {};
    console.log('connected to peer!');
    socketToPeer.on('multiplex-data', (channelId, message) => {
        console.log('\nMessage from peer', message.length);
        if (!sockets[channelId]) {
            sockets[channelId] = (0, net_1.createConnection)(portToForward);
            ...
            sockets[channelId].on('data', (data) => {
...
const setupSocketToPeer = (localPortUsedWithServer, peerAddress, portToForward, networkType, ownAbortController, otherAbortController) => new Promise((resolve, reject) => {
...
eventEmitter.on('socketReadyForPortForwarding', (socketToPeer) => {
